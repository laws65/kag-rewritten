...
func _physics_process(_delta: float) -> void:
	...
	velocity = get_parent().move_and_slide(velocity, FLOOR_NORMAL)
	_send_player_info()
...
func _flip_player() -> void:
	var mouse_position = sign(get_parent().get_global_mouse_position().x - get_parent().global_position.x)
	if mouse_position < 0:
...
func _send_player_info() -> void:
	if Server.is_connected:
		var state = {"T": Server.client_clock, # time
			 "P": get_parent().get_global_position(),  # position
